...
| Action system | Selectable idle pool (all non-speaking actions included by default; Speaking / TALK forced out), Idle / Speaking slots (optional Speaking secondary head/torso), shuffle-bag multi-clip cycling (every clip plays once per round before reshuffling; configurable idle gap, speaking chains without one), `loop` / `one-shot` / `pose`, preview/rename/move, MCP catalog |
...
The version on `main` is **`1.0.6`**; this version has not been released, so the formal stable release remains **[`v1.0.5`](https://github.com/SanHsien/voxavatar/releases/tag/v1.0.5)**. Version 1.0.6 adds a per-action-type idle pool: every non-speaking action is included by default, while Speaking / TALK and the action bound to Speaking are forced out. Version 1.0.5 upgraded the unsupported Electron 39 line, which pulled in vulnerable `extract-zip`, to supported Electron 43.4.0; the old extractor is absent from the lockfile, and the full dependency audit plus an anti-regression test now run in the standard gate. The formal runner completed the licensed-asset gate, Node 24 check, Windows native build/self-test, and NSIS packaging; the GitHub digest, checksum, and local SHA-256 agree. A clean per-user installation on Windows 11 at 225% DPI also confirmed AvatarSample_A as the default, zero custom models/actions, and Settings MCP Online/Ready with six tools and 12 actions. The four bundled VRMs and 13 VRMAs retain per-file license, quality-`keep`, and digest review. The repository has left its fork network while retaining upstream credit and the local `upstream` remote. Public installers remain Authenticode `NotSigned`; the unverified portions of the full DPI, SmartScreen, signing, and exporter matrix stay explicit. Former `REVIEW.md` lives under [`ROADMAP.md`](ROADMAP.md) “Current health”. Upstream evaluation: [`docs/DECISIONS.md`](docs/DECISIONS.md) §1 (Traditional Chinese).

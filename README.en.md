...
The version on `main` is **`1.0.5`** (not yet released); the current formal stable release remains **[`v1.0.4`](https://github.com/SanHsien/voxavatar/releases/tag/v1.0.4)**. The Windows-only, local-first product and integration contracts are stable. `1.0.5` upgrades the unsupported Electron 39 line, which pulled in vulnerable `extract-zip`, to supported Electron 43.4.0; the old extractor is absent from the lockfile, and the full dependency audit plus an anti-regression test now run in the standard gate. `v1.0.4` bundles four VRMs and 13 VRMAs after per-file license, quality-`keep`, and digest review; its formal runner, digest, and checksum evidence remains versioned. The GitHub repository has left its fork network while retaining upstream credit and the local `upstream` remote. Public installers remain Authenticode `NotSigned`; DPI, SmartScreen, signing, and exporter sub-items without evidence stay explicitly unverified. Former `REVIEW.md` lives under [`ROADMAP.md`](ROADMAP.md) “Current health”. Upstream evaluation: [`docs/DECISIONS.md`](docs/DECISIONS.md) §1 (Traditional Chinese).

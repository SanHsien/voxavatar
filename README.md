...
`main` 目前版本為 **`0.16.1`**；GitHub repo 已解除 fork network，並持續保留上游 credit 與 `upstream` remote。GitHub Latest Release 仍為 **`v0.16.0`**，本輪不建立安裝版 Release。GUI smoke／簽署仍標未驗。原 `REVIEW.md` 已併入 [`ROADMAP.md`](ROADMAP.md)「目前健康」。上游評估見 [`docs/DECISIONS.md`](docs/DECISIONS.md) §1。

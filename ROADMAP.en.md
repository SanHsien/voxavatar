...
Updated: 2026-08-14
Planning baseline: `1.0.4` (`main`; not released; GitHub Latest Release: `v1.0.2`; upstream eval in [`docs/DECISIONS.md`](docs/DECISIONS.md) §1)
...
Review baseline: `1.0.4` / `main`; GitHub Latest Release: `v1.0.2`
...
- `main` 1.0.4: VRoid Sample A/B/C, Tsukuyomi-chan Type A, and 13 CC0 VRMAs all pass source, redistribution, quality-`keep` (default >75; 75 is `review`), and SHA-256 review. Idle, Speaking, and ten custom actions work out of the box; `assets:release` recomputes every digest.
...
The 1.0.4 candidate used Node 24.19.0 for lint, 305 Node tests, 152 renderer tests, the production build, the release asset gate, and a local `electron-builder` NSIS package; the packaged app contains exactly four VRMs and 13 VRMAs. C++ Build Tools are not installed locally, so the 1.0.4 native rebuild / self-test and formal GitHub-runner NSIS gate remain pending until CI runs after push. A Windows 11, 225% DPI fresh-userData smoke rendered AvatarSample_A and listed the complete catalog; no claim is expanded to 100% / 150% DPI, switching the other three models, or per-clip playback.

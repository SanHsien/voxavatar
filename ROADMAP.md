...
更新日期：2026-08-14
規劃基準：`1.0.5`（`main`；尚未 Release；GitHub Latest Release：`v1.0.4`；上游評估見 [`docs/DECISIONS.md`](docs/DECISIONS.md) §1）
...
覆核基準：`1.0.5`／`main`；GitHub Latest Release：`v1.0.4`
...
- `main` 1.0.5：Electron 39.8.10 升級至仍受支援的 43.4.0；GHSA-jmr9-qjv8-65gv 的 `extract-zip@2.0.1` 已由 lockfile 移除，改用 Electron 維護的 extractor。完整 dependency audit 為 0，且日常／CI gate 不再只檢查 production dependencies。
...
| v1.0.5 | 升級至受支援的 Electron 43；移除易受 symlink path traversal 影響的 `extract-zip`，完整 dependency audit 納入 CI |

...
更新日期：2026-08-02
規劃基準：`0.16.8`（`main`；GitHub Latest Release tag 仍為 `v0.16.0`；上游評估見 [`docs/DECISIONS.md`](docs/DECISIONS.md) §1）
...
覆核基準：`0.16.8`／`main`；GitHub Latest Release：`v0.16.0`

沒有已知未解 P0／P1。上游 open PR／issue 已評估（無須合併，見 [`docs/DECISIONS.md`](docs/DECISIONS.md) §1）。**本輪不規劃新功能路線**；關閉既有缺口與契約漂移。`main` tip `0.16.8`：未分類片段池、可讀磁碟檔名、批次用途。

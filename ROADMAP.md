...
更新日期：2026-08-02
規劃基準：`v0.15.2`（`main` tip；正式 Release tag `v0.13.0`；上游評估見 [`docs/DECISIONS.md`](docs/DECISIONS.md) §1）
...
覆核基準：`v0.15.2`／`main`；GitHub Latest Release：`v0.13.0`

沒有已知未解 P0／P1。`v0.13.0` 為 Latest。上游 open PR／issue 已評估（無須合併，見 [`docs/DECISIONS.md`](docs/DECISIONS.md) §1）。**本輪不規劃新功能**；對齊 docs 承諾與實作。`main` tip `0.15.2`：文件／整合缺口收斂（未另 tag）。

- Latest Release：`v0.13.0`；`main` tip 為 `0.15.2`。
...
| 使用者手動狀態 UI | **未做** | 仲裁保留 `user` 最高優先；**無設定頁／系統匣入口**（不做新功能前僅註記） |
...
- [~] Native COM／WASAPI 分型 exit code（Windows runner）。
- [~] Idle／DPI／30%／GUI smoke／Installer 簽署（見驗證缺口表）。
- [~] 真實 exporter 人工結果。
- [~] 使用者手動狀態 UI（仲裁已支援；無產品入口）。

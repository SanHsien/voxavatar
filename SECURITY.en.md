...
- MCP tool results are versioned JSON (`status_schema_version` / `tools_schema_version`); agents should read structured fields — see [`docs/INTEGRATIONS.md`](docs/INTEGRATIONS.md). `get_status` and the Settings voice-source catalog redact paths/usernames in `listener.error` / `source` (and catalog `error`); prefer `helper_error` / `state`.

...
- Settings 狀態槽／品質門檻／語音模式與氣泡錨點有 jsdom 互動測；目錄／action-pack partial failure 有可見 notice；動作頁未分類池／依檔名分槽有 static 測。
- 離線 `vrma:curate`（inspect／rename／verify-names）有 Node 測試；真實素材只記授權清楚的人工結果。

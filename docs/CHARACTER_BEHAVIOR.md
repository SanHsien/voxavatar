...
動作包可用薄的 [`action-pack.json`](ACTION_PACK.md) 描述名稱、用途、狀態槽與檔案參照；它不能繞過既有匯入驗證、授權 gate 或 app-controlled 資產路徑。狀態槽名稱解析見 `src/character-state-slots.ts`（系統槽 UI／實際匯入管線仍待）。

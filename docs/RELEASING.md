...
用 `npm run evidence:manifest -- --version <version> [--installer-name …] [--installer-sha256 …] [--installer-size …] [--smoke-md] [--no-installer]` 建立機讀的 `manifest.json`，可選 `windows-smoke.md` 骨架。`--no-installer`（tip）時 `tag` 預設為 `null`，除非顯式傳 `--tag`。每個版本只保留實際執行結果；無法執行的項目寫「未驗」與原因，不可預先勾選通過。證據目錄不是 installer、log dump 或私人媒體的保存位置。

...
> `smokeExecuted=false` because the full Windows lifecycle matrix remains incomplete. Formal GitHub runner assets and workflows are verified, and the downloaded installer completed a desktop same-version reinstall; partial and unverified rows remain explicit below.
...
- desktop execution: pass — 正式下載檔完成同版本重裝並自動重啟；另保留同 commit 本機候選的先行證據（`105138679` bytes，SHA-256 `5b8758b15f82d85a0e6af51539ff3160454992a36479b01a20c9c605b1e64acb`）
...
- [ ] **安裝** (`install`): 部分 — 正式 GitHub installer 在既有「所有使用者」／Program Files 安裝上完成同版本重裝並自動啟動；ProductVersion `1.0.0.0`／FileVersion `1.0.0`，12 個模型、10 個動作、預設模型、語音自動模式、訊息氣泡開關與 2 個 MCP 動作均保留，readiness complete，bridge 只綁 `127.0.0.1`。全新安裝、自訂位置、捷徑與一般使用者免管理員矩陣仍未驗。
...
- 正式重裝後實機發現 Settings MCP 卡片即使服務已 online 仍永久顯示「啟動中」。根因是 Settings IPC 在 bridge `listen()` 前閉包 health／port／error／listener primitive；1.0.2 改為每次呼叫讀取 live runtime state，並以 `starting:0`→`online:49152` 回歸測釘住。

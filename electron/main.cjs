...
function applyUserCharacterState(state) {
  return applyCharacterState(
    { state },
    { sessionId: TRAY_USER_SOURCE_ID, sourceKind: "user" },
  );
}

function clearUserCharacterState() {
  clearCharacterStateForSource(TRAY_USER_SOURCE_ID);
  return true;
}

function buildCharacterStateMenuSubmenu(t) {
  return buildCharacterStateMenuEntries(t, {
    onSelect: (state) => {
      applyUserCharacterState(state);
    },
    onClear: () => {
      clearUserCharacterState();
    },
  });
}

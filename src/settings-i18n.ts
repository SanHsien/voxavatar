...
const ZH_TW: MessageDictionary = {
  ...
  'actions.clipsManageHint':
    '點片段或「預覽」可在右側預覽播放；「編輯」可改顯示名稱、用途（loop／one-shot／pose）或移到其他動作。磁碟仍以安全 UUID 儲存，不會改寫原始檔案路徑。',
  ...
  'actions.clipEditHint':
    '顯示名稱請用小寫英文、數字與連字號（例如 wave-soft）。用途會影響品質分析與播放語意。',
...
const EN: MessageDictionary = {
  ...
  'actions.clipsManageHint':
    'Click a clip or Preview to play on the right. Edit renames the display name, sets purpose (loop / one-shot / pose), or moves the clip to another action. Files stay as safe UUID names on disk.',
  ...
  'actions.clipEditHint':
    'Display names use lowercase letters, numbers, and hyphens (e.g. wave-soft). Purpose affects quality analysis and playback semantics.',
